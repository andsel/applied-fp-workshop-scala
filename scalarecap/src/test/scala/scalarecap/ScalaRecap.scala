...
class ScalaRecap extends munit.FunSuite {
  ...
  case class Person(name: String, age: Int) {
    ...
    def makeOlder(years: Int) = Person(name, age + years)

    def makeYounger(implicit years: Int) = Person(name, age - years)

  }

  object Person {
    def create(encoded: String): Person = {
      val splitted = encoded.split(';')
      Person(splitted(0), splitted(1).toInt)
    }

    def apply(encoded: String) = create(encoded)

    def isFake(person: Person) = {
      person match {
  ...
  trait Fruit {
    ...
    def eatenBy(eater: String) = s"$eater ate $stringify"
  }
